...
def main():
    ...
    while True:
        choice = ''
        # Loop until a valid choice (S, U, Q) is entered
        while choice not in ['S', 'U', 'Q']:
            choice = input("\nDo you want to (S)crambler, (U)nscramble, or (Q)uit? (S/U/Q): ").strip().upper()
            if choice not in ['S', 'U', 'Q']:
                print("Invalid choice. Please enter 'S' for Scramble, 'U' for Unscramble, or 'Q' to Quit.")
